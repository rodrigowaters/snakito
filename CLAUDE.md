...
- [~] M3 (antigo M2) — em andamento. Entregas fechadas:
  ...
  - **AdMob recompensado ✅ (17/08)** — autoload `Anuncios` (`src/scenes/anuncios.gd`), única porta do AdMob e **só o formato recompensado** (é o que os blueprints usam; banner/interstitial ficam fora até o design prever lugar). Política de Famílias ANTES do `initialize`: `tagForChildDirectedTreatment` + TFUA + classificação G, com fluxo UMP; App ID REAL em `project.godot` (`admob/general/android/app_id`, conta criada em 20/08) e injetado no manifest pelo export; os BLOCOS ficam em `anuncios.gd` — debug SEMPRE no bloco de teste (produção em playtest = suspensão por tráfego inválido), release usa `ID_RECOMPENSADO_PRODUCAO` e, vazio, não serve anúncio em vez de cair no de teste. Bloco **Premiado** `.../1017525581` criado em 20/08 (formato de opt-in explícito — o Premiado exige aceite, o "intersticial premiado" não; nossas telas pedem o aceite). COPPA/TFUA/G **verificados na requisição real (20/08)**: `gdpr=0&tfcd=1&tfua=1`. Fluxo completo comprovado no aparelho — exibiu, recompensa concedida, renasceu, próximo anúncio pré-carregado; caminho do TICKET também validado (saldo 3→2, renasce sem vídeo). **Saída que não pode agir NÃO é exibida** (playtest 20/08: "prefiro não exibir o botão do que mostrar ele desabilitado") — sem chance restante, sem anúncio em estoque ou sem ticket, o botão não existe e o texto para de prometer renascimento; vale para os dois botões e para a pílula do VIP. **Entitlement `ads_removed` ligado (regra dura #6)**: `Rede.atualizar_entitlements()` lê a tabela junto do perfil e espelha em `ProgressoLocal` para valer OFFLINE (regra #5; a lista nova sobrescreve a seção — reembolso revoga). Com o entitlement, NENHUM anúncio é carregado e nenhum botão de anúncio é renderizado. Leitura registrada: **"Remover anúncios" remove ANÚNCIOS, não CUSTOS** — onde o anúncio era o preço (subir buff, renascer), moedas e tickets seguem valendo; renascer grátis é perk de VIP (tokens), não de `ads_removed`. **Privacidade do UMP** na tela 10: a linha abre `show_privacy_options_form` só quando o UMP exige revisão. O 04b reage a `disponibilidade_mudou` (anúncio que chega depois da modal abrir aparece). **Faltam no console**: mensagem de privacidade/consentimento do UMP (o log aponta `Publisher misconfiguration: no form(s) configured for the input app ID` — o app funciona, mas a Europa fica sem formulário) e marcar o app como dirigido a crianças. Fora do Android o serviço fica inerte e a UI degrada para os placeholders. Consumidores: Loja/Buffs ("▶ ANÚNCIO" sobe 1 nível sem moedas; "🎟️ Pular" entrega a mesma recompensa sem vídeo — saída offline dos tickets) e Renascimento 04b
  ...
  - **Billing — fundação pronta (21/08)**: autoload `Compras` (`src/scenes/compras.gd`) é a única porta do Play. Regra que organiza tudo: **o Play cobra, o SERVIDOR concede** — recibo vai para a Edge Function `validate_purchase`, que confere com a Google Play Developer API (JWT de service account), grava a razão `purchases` (migration 0007; `purchase_token` é PK = idempotência e proveniência) e o `entitlements`; só depois do 200 o cliente credita saldo local. `acknowledge`/`consume` acontecem DEPOIS do servidor (recibo não reconhecido em 3 dias é reembolsado pelo Play — rede de segurança do jogador se a nossa validação quebrar). `FilaRecibos` (criptografada) segura o que não validou. **Preço NUNCA é hardcoded**: vem de `query_product_details` na moeda do país. Catálogo em `catalogo_produtos.gd` espelha o da função (teste `test_catalogo_bate_com_a_edge_function` lê o .ts e compara). "Restaurar compras" (tela 10) chama `query_purchases`. **Trava honesta**: pacotes de SKIN estão `vendavel = false` — sem render de padrão em jogo, vender skin que não veste seria cobrar por nada (teste trava isso). **Faltam**: deploy da migration 0007 + função (MCP do Supabase estava desconectado), secret `GOOGLE_SERVICE_ACCOUNT_JSON`, produtos no Play Console, faixa interna + upload key para testar compra real, e o render de padrão premium em jogo
  - **Faltam**: (ex-Billing — produtos reais dos Pacotes + "Remover anúncios"; o entitlement já é lido pelo cliente e o plugin está vendorado desde o spike, então falta criar os produtos no Play Console, a Edge Function de validação de recibo e "Restaurar compras"), sons (os toggles de Configurações já persistem esperando), analyzer completo, EN/ES, publicação
  - **Decisões de gameplay aguardando veredito**: uma chance de renascer por partida (`GameEngine.RENASCIMENTOS_MAX`) e o nível caindo à metade ao renascer (`FRACAO_NIVEL_AO_RENASCER`) — ambas são interpretações nossas, o design só fixa "mantém os pontos"
