...
- [~] M3 (antigo M2) — em andamento: **Configurações (10) ✅ (13/08)** — layout fixo sem scroll (2 rodadas: gesto briga com botões de linha na rota touch→mouse; scroll volta na Loja com validação em aparelho); funcionais: Vibração, **modo daltonismo ✅** (símbolos geométricos por cor, cabeça + a cada 3 segmentos — spec dos tokens), Sair/Excluir conta (confirmação inline); guardam lugar: sliders de áudio, Idioma, Privacidade, Remover anúncios/Restaurar compras; dificuldade do Arcade SEM linha (não está no desenho — interna, padrão CHEIA). Tela 02b Info do jogador ✅ (avatar da Home; stats locais acumuladas; SEM porta p/ Conta — Configurações é a porta). Faltam: AdMob, Billing (+ Loja 09*), sons, analyzer completo, desafios 3–4, EN/ES, publicação
